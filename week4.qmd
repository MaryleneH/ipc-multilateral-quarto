...
# --- Coercitions robustes depuis R -> Python ---
def as_array(x):
    # Accepte numpy array, pandas DataFrame/Series, ou liste imbriquée
...
P = as_array(price_data)
Q = as_array(quantity_data)
...
n_items, Tt = P.shape
assert Tt >= 2, "Il faut au moins 2 périodes"
assert Q.shape == P.shape, "quantity_data doit avoir la même forme que price_data"

# ---------- Données longues ----------
cpd_df = pd.DataFrame({
    "period": np.repeat(np.arange(Tt), n_items),
    "item":   np.tile(np.arange(n_items), Tt)
})
cpd_df["price"] = P[cpd_df["item"], cpd_df["period"]]
cpd_df["log_price"] = np.log(cpd_df["price"])

# ---------- CPD : intercept + dummies (base = période 0, item 0) ----------
period_cat = pd.Categorical(cpd_df["period"], categories=range(Tt), ordered=True)
item_cat   = pd.Categorical(cpd_df["item"],   categories=range(n_items), ordered=True)
period_d   = pd.get_dummies(period_cat, prefix="period", drop_first=True)
item_d     = pd.get_dummies(item_cat,   prefix="item",   drop_first=True)

X = pd.concat([item_d, period_d], axis=1)
X = sm.add_constant(X)  # intercept
y = cpd_df["log_price"]

model = sm.OLS(y, X).fit()

# Effets de période (référence = 0)
period_cols = sorted([c for c in X.columns if c.startswith("period_")],
                     key=lambda c: int(c.split("_")[1]))
beta = np.zeros(Tt)  # beta[0] = 0
for k, col in enumerate(period_cols, start=1):
    beta[k] = float(model.params.get(col, 0.0))
cpd_indices = np.exp(beta - beta[0])

print("Forme P (items x périodes):", P.shape)
print("Indices CPD:", np.round(cpd_indices, 6))
print("R-squared:", round(model.rsquared, 4))

# ---------- Bilatéral Fisher + GEKS (général pour Tt périodes) ----------
def fisher_bilateral(p_s, p_j, q_s, q_j):
    L = (p_j * q_s).sum() / (p_s * q_s).sum()
    P_ = (p_j * q_j).sum() / (p_s * q_j).sum()
    return np.sqrt(L * P_)
...
for s in range(Tt):
    for j in range(Tt):
        if s != j:
            B[s, j] = fisher_bilateral(P[:, s], P[:, j], Q[:, s], Q[:, j])

# GEKS(t) = exp( mean_k log B[t,k] - mean_k log B[0,k] )
geks = np.exp(np.log(B).mean(axis=1) - np.log(B[0]).mean())

# ---------- Viz ----------
periods = np.arange(1, Tt+1)
plt.figure(figsize=(9,5))
plt.plot(periods, cpd_indices, 'o-', linewidth=2, markersize=8, label="CPD")
plt.plot(periods, geks,       's-', linewidth=2, markersize=8, label="GEKS")
plt.xlabel("Période"); plt.ylabel("Indice (Période 1 = 1.00)")
plt.title("CPD vs GEKS"); plt.legend(); plt.grid(alpha=.3); plt.tight_layout()
plt.show()
...
3. Compare CPD with GEKS-Törnqvist
